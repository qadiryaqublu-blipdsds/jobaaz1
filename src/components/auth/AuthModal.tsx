...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  // -------------------------------------------------------------
  // 0. Google Firebase Direct Sign-In
  // -------------------------------------------------------------
  const handleGoogleSignIn = async () => {
    ...
    setLoading(true);
    try {
      const targetRole = mode === 'register' && registerRole === 'business' ? 'business' : 'candidate';
      const { user, session } = await loginWithGoogleFirebase(targetRole);
      setSuccessMsg(`Uğurla daxil oldunuz! Xoş gəldiniz, ${user.fullName || user.email}!`);
      setTimeout(() => {
      ...
      if (err?.code === 'auth/popup-closed-by-user' || err?.code === 'auth/cancelled-popup-request') {
        // User closed or cancelled popup, do not show an intrusive error banner
        setErrorMsg(null);
      } else if (err?.code === 'auth/popup-blocked') {
        setErrorMsg('Brauzer pop-up pəncərənin açılmasını blokladı. Zəhmət olmasa pop-up pəncərələrə icazə verin və ya tətbiqi yeni tabda açın.');
      } else {
        setErrorMsg(err?.message || 'Google ilə giriş zamanı xəta baş verdi.');
      }
    } finally {
      setLoading(false);
    }
          ...
          {mode === 'login' && (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full py-2.5 px-4 bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-xl text-xs font-bold text-slate-700 shadow-xs transition-all flex items-center justify-center gap-2.5 cursor-pointer disabled:opacity-60"
              >
                ...
                <span>Google ilə bir toxunuşla daxil ol</span>
              </button>
              ...
              <div className="relative my-2">
                ...
                <div className="relative flex justify-center text-[10px] uppercase">
                  <span className="bg-white px-2 text-slate-400 font-semibold tracking-wider">və ya e-poçt ilə</span>
                </div>
              </div>
          ...
          {mode === 'register' && (
            <div className="space-y-4">
              ...
              >
                ...
                <span>Google ilə {registerRole === 'business' ? 'İşəgötürən' : 'Namizəd'} hesabı aç</span>
              </button>
              ...
              <div className="relative my-2">
                ...
                <div className="relative flex justify-center text-[10px] uppercase">
                  <span className="bg-white px-2 text-slate-400 font-semibold tracking-wider">və ya form dolduraraq</span>
                </div>
              </div>
